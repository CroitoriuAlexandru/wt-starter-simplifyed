...
ElemNode::ElemNode(std::shared_ptr<XMLBrain> xml_brain, tinyxml2::XMLNode *node, std::shared_ptr<XMLBrain> top_parent_xml_template)
      ...
      node_(node)
{
    ...
    else if (node_->ToText())
    {
        ...
        if (temp_var_data.name_ != "")
        {
            if (temp_var_data.widget_type_ == TempVarWidgetType::TEMPLATE)
            {
                ...
                {
                    addWidget(std::make_unique<ElemNode>(child_xml_brain, child_xml_brain->message_node_, top_parent_xml_template_));
                }
                // auto child_temp_xml_brain = xml_brain_->stylus_brain_->getXMLBrain(widget_text);
                // if(child_temp_xml_brain != nullptr)
                // {
                // auto child_temp = addWidget(std::make_unique<ElemNode>(child_temp_xml_brain, child_temp_xml_brain->message_node_));
    ...
    if (node_->FirstChild())
    {
        ...
        {
            auto child_ElemNode = addWidget(std::make_unique<ElemNode>(xml_brain_, child, top_parent_xml_template_));
        }
    }
}
