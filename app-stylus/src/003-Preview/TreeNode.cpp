...
TreeNode::TreeNode(std::shared_ptr<XMLBrain> xml_brain, tinyxml2::XMLNode *node)
      ...
      node_(node)
{
    ...
    if (node->FirstChild())
    {
        for (auto child = node->FirstChild(); child; child = child->NextSibling())
        {
            if (child->ToElement())
            {
                content_->addWidget(std::make_unique<TreeNode>(xml_brain_, child));
            }
            else if (child->ToText())
            {
                titleBar_->addWidget(std::make_unique<TreeNode>(xml_brain_, child));
            }
        }
    }
}